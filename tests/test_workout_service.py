    ...
    def test_generate_workout_plan(self, mock_generate_workout_plan):
        ...
        mock_response = {
            ...
            "workout_sessions": [
                {
                    "exercises": [
                        {"name": "Barbell Bench Press", "sets": 4, "reps": "8-12", "rest": 60},
                        {"name": "Barbell Squat", "sets": 4, "reps": "8-12", "rest": 60},
                        {"name": "Barbell Deadlift", "sets": 1, "reps": "5-8", "rest": 120},
                        {"name": "Dumbbell Rows", "sets": 3, "reps": "10-15", "rest": 45},
                        {"name": "Overhead Press", "sets": 3, "reps": "10-15", "rest": 45}
                    ]
                },
                {
                    "exercises": [
                        {"name": "Dumbbell Bench Press", "sets": 4, "reps": "8-12", "rest": 60},
                        {"name": "Leg Press", "sets": 4, "reps": "10-15", "rest": 45},
                        {"name": "Pull-ups", "sets": 3, "reps": "8-12", "rest": 60},
                        {"name": "Dumbbell Shoulder Press", "sets": 3, "reps": "10-15", "rest": 45},
                        {"name": "Dumbbell Bicep Curls", "sets": 3, "reps": "10-15", "rest": 45}
                    ]
                }
            ],
            "cooldown": {
        ...
        for session_index, session in enumerate(data["workout_sessions"]):
            print(f"Session {session_index} data: {json.dumps(session, indent=2)}")  # Debugging line
            for exercise_index, exercise in enumerate(session["exercises"]):
                print(f"Exercise {exercise_index} data: {json.dumps(exercise, indent=2)}")  # Debugging line
                assert exercise["name"] == mock_response["workout_sessions"][session_index]["exercises"][exercise_index]["name"]
                assert exercise["sets"] == mock_response["workout_sessions"][session_index]["exercises"][exercise_index]["sets"]
                assert exercise["reps"] == mock_response["workout_sessions"][session_index]["exercises"][exercise_index]["reps"]
                assert exercise["rest"] == mock_response["workout_sessions"][session_index]["exercises"][exercise_index]["rest"]

        assert "cooldown" in data
        assert data["cooldown"]["description"] == mock_response["cooldown"]["description"]

...
def analyze_meal(image_data: bytes) -> Meal:
    ...
    try:
        ...
        logging.info(f"Parsed Food Name: {food_name}, Total Calories: {total_calories}, Calories per Ingredient: {calories_per_ingredient}")

        if food_name is None or total_calories is None or not calories_per_ingredient:
            logging.error("Missing food name, total calories, or calories per ingredient in the response")
            raise HTTPException(status_code=500, detail="Missing food name, total calories, or calories per ingredient in the response")

    except Exception as e:
        logging.error(f"Exception (Analyze Meal): {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
    
    return Meal(food_name=food_name, total_calories=total_calories, calories_per_ingredient=calories_per_ingredient)
